...
remote_arch=linux_arm
watch_port_check_seconds=360
remote_url='https://api.github.com/repos/autobrr/autobrr/releases/latest'
get_ui_listening_address_cmd="/bin/grep '^host ' '$qpkg_ini_pathfile'|cut -d '=' -f2|tr -d ' \"'"
get_ui_port_cmd="/bin/grep '^port ' '$qpkg_ini_pathfile'|cut -d '=' -f2|tr -d ' \"'"
